...
\coro{Corol·lari 10}{Sigui $f$ d'ordre $\alpha$ amb $f(0)\neq 0$. Aleshores per tot $\rho\in\RR_{>\alpha}$, tenim:
\begin{enumerate}
    \item $n(R)=\bigO(R^{\rho})$ per tot $R\gg1$; i on $n:\RR_{\geq0}\to \NN$ és el número de zeros amb mòdul menor que $R$.
    \item $\sum_{n\in N_f}|s_n|^{-1}$, on $s_n$ és el $n$-èsim zero de $f$ (indexat per $N_f$).
\end{enumerate}
    }
\coro{Corol·lari 14}{Sigui $f$ d'ordre 1 amb $f(0)\neq0$. Si $\sum_{n\in N_f}|s_n|^{-1}$ convergeix, aleshores $|f|<e^{C|s|}$ per $s\gg1$.}
\pf{
...
\]
Notem que l'ordre dels zeros trivials de $R_1$ és 1 (i els no trivials els sumem tantes vegades com l'ordre del zero); i els altres residus són:
\begin{align*}
