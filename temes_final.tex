...
\pf{
    ...
    \[
        a_n=\frac{1}{2\pi R^n}\int_0^{2\pi}(g(Re^{i\theta})+\ol{g(Re^{i\theta})})e^{-in\theta}\dd =\frac{1}{\pi R^n}\int_0^{2\pi}\Re (g(Re^{i\theta})e^{-in\theta})\dd 
    \]
...
Notem que els zeros de la funció $xi$ són exactament els zeros (amb multiplicitt) de $\zeta$ en la franja crítica.
\pf{
    ...
    En resum $|\xi(s)|=\bigO(e^{|s|^{1+\eps}})$ $\eps\forall>0$, $|s|\gg1$ i $\s\geq\half$. I per l'equació funcional de $\xi$, ho tenim per $|s|\gg1$. És a dir, que $\xi$ és d'ordre $\leq$1. I per tant, ara ens fa falta veure que l'ordre és exactament 1.\\[4mm]
    ...
    \]}
    O sigui, que en els enters, sabem que la funció $\Lambda$ és gran:
    \[
        \log(\xi)=\log(\Lambda)-\frac s2\log(\pi)+\log\left(\half s(s-1)\right)+\log(\zeta)
    \]
    Però en $s=r$: sabem que $\log\Lambda(r)=\log(r-1)!\sim r\log r$. També sabem que $\log\zeta(r)=\log\sum_{n\geq1}\frac{1}{n^r}\xrightarrow{r\to\infty}0$ (ja que $\sum_{n\geq1}\frac{1}{n^r}\to 1$.). I finalment la resta de coses són de la mida $-r/2\log(\pi)$. Per tant
    \[
