...
\teorema{equació funcional de $\theta(x)$}{\ref{4.02_teorema equació funcional de theta} La funció $\theta(x)$ de Jacobi satisfà la següent equació funcional:
\[
\theta\left(\frac{1}{x}\right)=\sqrt{x}\,\theta(x)
\]
}
Per fer aquesta demostració necessitarem resultats de l'assignatura d'anàlisi matemàtica:
\teorema{Expansió de Fourier.}{\ref{4.13_teorema resultat d'anmat}Sigui $f\colon \RR\to \RR$ una funció contínua i monòtona a trossos. Descrivim
\[
...
\end{array}\right.
\]
Aleshores, la funció es pot expressar de la següent manera:
\[
f_1(u)=\frac{a_0}{2}+\sum_{\nu\geq1}(a_{\nu}\cos(2\pi \nu u)+b_\nu\sin(2\pi \nu u))
\]
On els coeficients venen donats per les següents fórmules:
\[
...
\teorema{Fórmula de Poisson summation.}{\ref{4.14_teorema Poisson summation} Sigui $f$ una funció 
