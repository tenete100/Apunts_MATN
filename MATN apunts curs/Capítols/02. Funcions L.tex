...
%diagrama commutatiu
commuta. L'isomorfisme, doncs, ve donat l'assignació $\psi \mapsto \psi'$, i té inversa $\psi' \mapsto \psi' \circ \pi$ on $\pi \colon G \to G/H$ és la
projecció al quocient.}
...
\]
Per tant, sabem que hi ha $\varphi(m)\over f$ caràcters que restringits al grup $\langle p\rangle$ són iguals, i envien $p\mapsto\xi$; per tant:
\[
\prod_\chi1-\chi(p)T=\prod_{\substack{\xi\text{ arrel $f$-èsima}\\\text{de la unitat}}}(1-\xi T)^{\varphi(m)\over f}
\]
Però el producte de $1-\xi T$ sobre totes les $f$-èsimes arrels de la unitat és $1-T^f$, per tant:
\[
\prod_\chi1-\chi(p)T=\prod_{\substack{\xi\text{ arrel $f$-èsima}\\\text{de la unitat}}}(1-\xi T)^{\varphi(m)\over f}=(1-T^f)^{\varphi(m)\over f}
\]
...
Valleé-Poussin va distingir 2 casos: si $\chi$ és real o si és complexa. Aleshores, nosaltres veurem primer:
...
\[
\zeta_m(s)\geq \sum_{\substack{n>1\\(n,m)=1}}\frac{1}{n^{\varphi}}
\]
