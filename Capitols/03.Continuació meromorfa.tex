...
\]
I ara, amb un exercici de anàlisi complexa:
...
\begin{tikzpicture}
    ...
    \node at (-2,-0.3) {$-R$};
    \node at (2,-0.3) {$R$};
    \node at (-0.2,2.2) {$\Ima(\beta)$};
\end{tikzpicture}
Notem que la funció $f(z)=\exp(-\pi x(z+\beta)^2)$ és una funció entera, per tant, per el teorema de Cauchy, tenim que:
\[
\int_{\gamma_1}f(z)\dd z+\int_{\gamma_2}f(z)\dd z+\int_{\gamma_3}f(z)\dd z+\int_{\gamma_4}f(z)\dd z=0
\]
Per tant, considerem ara les integrals corresponents a $\gamma_2$ i $\gamma_4$:
\[
