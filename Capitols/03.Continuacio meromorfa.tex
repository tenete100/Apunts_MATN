\chapter{\texorpdfstring{Continuació meromorfa i equació funcional de $\zeta(s)$}{Continuació meromorfa i equació funcional de zeta(s)}}
...
\teorema{equació funcional de $\theta(x)$}{3.02 equació funcional de theta}{
    La funció $\theta(x)$ de Jacobi satisfà la següent equació funcional:
    \[
...
\teorema{Expansió de Fourier.}{3.13 resultat d'anmat}{
...
\teorema{Fórmula de sumació de Poisson}{3.14 Poisson summation}{
    Sigui $f$ una funció com abans. Sigui $A$, $B\in \ZZ$, amb $A<B$. Aleshores:
    \[
...
I ara, amb aquests dos teoremes, podem donar la demostració del teorema \ref{Teorema:3.02 equació funcional de theta}.
\pf{
...
\dfe{Funció $\Gamma$}{
    La funció $\Gamma$, introduïda per Euler, es defineix com
    \[
        \Gamma(s)=\int_{0}^{\infty}e^{-x}x^s\frac{\dd x}{x}\qquad \text{per }\s>0
    \]
}
I no és gaire sorprenent, que aquesta funció que serà de gran rellevància, sigui holomorfa:
\lema{La funció $\Gamma$ és holomorfa.}{3.03 holomorfitat de gamma}{
    La funció $\Gamma(s)$ és holomorfa per $\sigma>0$.
}
\pf{
    ...
    \]
    Volem veure que $\Gamma_n$ és holomorfa; i que $\Gamma_n$ convergeix uniformement cap a $\Gamma$ sobre compactes dins de $\sigma>0$. Aleshores, amb el teorema \ref{Teorema:1.10} ens regalarà la holoformitat de $\Gamma$.\par
    \vspace{10mm}
...
\lema{Equació funcional de $\Gamma$.}{3.04 eq. funcional de Gamma}{
    La funció $\Gamma$ satisfà
    \[
...
\pf{
    Com que del lema \ref{Lema:3.04 eq. funcional de Gamma} tenim que $\Gamma(n)=(n-1)\Gamma(n-1)=\dots=(n-1)!\Gamma(1)$. Per tant, només hem de veure quant val $\Gamma(1)$:
    \[
