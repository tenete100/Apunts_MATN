\chapter{Funcions L}
Fins ara hem estat tractant amb funcions $F(s)=\sum_{n\geq1}\frac{f(n)}{n^s}$; i hem vist un cas (teorema \ref{Teorema:1.17 extensió_de_riemann}) de com
...
\dfe{El grup de caràcters}{
    Denotem per $\widehat{G}$ al grup $\widehat{G}=\Hom(G,\CC^*)=\{\text{Caràcters de G}\}$, on la operació és:
    \[
    \psi,\phi\colon G\rightarrow\CC^*\qquad\text{aleshores }(\psi{\;\cdot_{\widehat{G}} } \;\phi)(g)\mapsto\psi(g)\cdot_{\CC^*}\phi(g)
    \]
...
\lema{El grup de caràcters d'un grup cíclic}{2.01 caràcter de grups cíclics}{
    Sigui $G=\langle s\rangle$ un grup cíclic d'ordre $n$ i sigui $\mu_n$ el grup d'arrels $n$-èssimes de la unitat. Aleshores
    \[
        \begin{alignedat}{2}
...
\pf{
    Com que $\psi(s)^n = \psi(s^n) = \psi(1) = 1$, $\psi(s)$ és arrel $n$-èssima de la unitat. D'altra banda, el morfisme és injectiu perquè si $\psi_1, \psi_2 \in \widehat{G}$ i $\psi_1(s) = \psi_2(s)$, llavors $\psi_1(s^k) = \psi_1(s)^k = \psi_2(s)^k = \psi_1(s^k)$ per tot $s^k \in \langle s \rangle = G$. Finalment, per veure l'exhaustivitat, si $\omega \in \mu_n$, podem definir $\psi(s^k) = \omega^k$.
}
Resulta també natural preguntar-se ara sobre la relació entre els caràcters d'un grup i el dels seus subgrups. 

\Prop{Extensió de caràcters de subgrups}{2.02 extensió de caràcters de subgrups}{
    Sigui $H \leq G$ subgrup. Tot caràcter de $H$ estén a un caràcter de $G$.}
\pf{
    Si $H = G$, el resultat és tautològic. Suposem $H \neq G$, i sigui $x \in G \setminus H$. Com que $G$ és finit, n'hi ha prou amb veure que un caràcter $\psi \colon H \to \CC^\times$ estén a un de $H' := \langle H, x \rangle$. Atès que $G$ és abelià, tot element de $H'$ es pot escriure com $hx^a$ on $h \in H$ i $a \in \ZZ$. A més $|G/H| < + \infty$, per tant $[x]$ és d'ordre finit i existeix $n \in \ZZ^+$ tal que $[x]^n = [1]$, equivalentment $x^n \in H$. Prenem el mínim dels $n$ tals que això passi i fixem $\omega \in \CC^\times$ tal que $\psi(x^n) = \omega^n$. Definim 
    \[
        \begin{alignedat}{2}
...
\Prop{Restricció de caràcters de subgrups}{2.03 restricció de caràcters de subgrups}{
    La restricció $\mathrm{Res} \colon \widehat{G} \to \widehat{H}$ que envia $\psi$ a $\psi|_H$ és exhaustiva, i $\ker(\mathrm{Res}) \cong \widehat{G/H}$.
}
...
\end{wrapfigure}
\pf{
    Dir que aquesta aplicació és exhaustiva és equivalent a la proposició anterior. Per veure l'isomorfisme, tenim que $\psi \in \ker(\mathrm{Res})$ si i només si $\psi(H) = \{1\}$ o equivalentment $H \subseteq \ker(\psi)$. Però això últim es dona si i només si existeix $\psi' \colon G/H \to \CC^\times$ tal que $\psi$ factoritza a través de de $\psi'$, és a dir, el diagrama commuta. L'isomorfisme, doncs, ve donat l'assignació $\psi \mapsto \psi'$, i té inversa $\psi' \mapsto \psi' \circ \pi$ on $\pi \colon G \to G/H$ és la projecció al quocient.
}

Això ens dona, a més, l'ordre del grup de caràcters d'un determinat grup.

\coro{Ordre del grup de caràcters}{2.04 ordre del grup de caràcters}{
    Si $G$ és un grup abelià finit, $|G| = |\widehat{G}|$.
}
\pf{
    Procedim per inducció sobre l'ordre. Si $|G| = 1$, és trivial. Suposem $|G| > 1$. Pel teorema de Cauchy, existeix un subgrup $H \leq G$ d'ordre $p$, que és cíclic per ser d'ordre primer i per tant pel lema \ref{Lema:2.01 caràcter de grups cíclics}, $|\widehat{H}| = |\mu_p| = p = |H|$. D'altra banda, per la  proposició anterior i pel primer teorema d'isomorfia, $|\widehat{H}| = |\widehat{G}|/|\ker(\mathrm{Res})| = |\widehat{G}|/|\widehat{G/H}|$. Ara, com que $|G/H| < |G|$, per la hipòtesi d'inducció $|G/H| = |\widehat{G/H}|$, i tot plegat deduïm
    \[
...
\Prop{Isomorfisme canònic entre $G$ i $\skew{3}\widehat{\widehat{G}}$}{2.05 G és isomorf a el dual del dual}{
    Definim $\epsilon \colon G \to \skew{3}\widehat{\widehat{G}}$ per $g \mapsto (\mathrm{ev_g}\colon \widehat{G} \to \mathbb{C}^\times)$ on $\mathrm{ev_g}(\psi) := \psi(g)$. Llavors $\epsilon$ és un isomorfisme.
}
\pf{
    Veure que és morfisme és directe: si $g, h \in G$ i $\psi \in \widehat{G}$, $\mathrm{ev}_{gh}(\psi) = \psi(gh) = \psi(g)\psi(h) = \mathrm{ev}_g(\psi) \mathrm{ev}_h(\psi)$, d'on $\mathrm{ev}_{gh} = \mathrm{ev}_g \mathrm{ev}_h$, és a dir, $\epsilon (gh) = \epsilon(g)\epsilon(h)$. A més, pel corol·lari anterior, $|G| = |\widehat{G}| = |\skew{3}\widehat{\widehat{G}}|$, per tant, per provar que és un isomorfisme només cal provar la injectivitat. Sigui $g \in \ker(\epsilon)$, és a dir, $\psi(g) = 1$ per tot $\psi \in \widehat{G}$. Suposem per arribar a contradicció que $g \neq 1$. Llavors tenim un caràcter $\psi \colon \langle g \rangle \to \CC^\times$ definit per $g \mapsto e^{\frac{2\pi i}{|\langle g\rangle|}} \neq 1$. Però llavors $\psi'$ estén a un caràcter $\psi$ de $G$, absurd ja que $\psi(g) = \psi'(g) \neq 1$.
}
...
\Prop{Relacions d'ortogonalitat}{2.06 relacions d'ortogonalitat}{
    \begin{itemize}
...
\pf{
    (i) Si $\psi = 1$, és directe. Suposem que existeix $h \in G$ tal que $\psi(h) \neq 1$. Aleshores
    \[
...
\exemple{
    Sigui $m=p$ primer senar. Llavors $(\mathbb{Z}/m\mathbb{Z})^\times$ és cíclic d'ordre $p-1$, i llavors té un únic element d'ordre 2. Per tant,
    \[
        \begin{alignedat}{2}
...
\dfe{Caràcter principal mòdul $m$}{
    El caràcter principal mòdul $m$ és $\chi_0(a) \colon \ZZ^+ \to \CC$ definit per 
    \[
        \chi_0(a) = \begin{cases}
            1 & \gcd(a, m) = 1 \\
            0 & \text{altrament}
...
\dfe{Funció $L$ associada a un caràcter de Dirichlet}{
...
\lema{Convergència de les funcions $L$ de caràcters no principals}{2.07 convergencia de funcions L(X,s), on X no és principal}{
...
\pf{
    (i) Sigui $x \geq 1$ i sigui $k \in \ZZ^+$ tal que $km \leq x \leq (k+1)m$. Aleshores, 
    \[
...
\lema{Continuació meromorfa de $L(\chi_0,s)$}{2.08 cont meromorfa de X_0}{
...
\pf{
    ...
    \ref{Corolari:1.05 de sèries de Dirichlet multiplicatives a productes d'Euler}, tenim que:
    \[
...
Notem també, que pel lema \ref{Lema:2.07 convergencia de funcions L(X,s), on X no és principal}, tenim que $L(\chi,1)\in\CC$. I el punt clau de la demostració
del teorema de la progressió aritmètica de Dirichlet consistirà en veure que $L(\chi,1)\neq0$ per cap caràcter de Dirichlet.
\lema{Un producte sobre tots els caràcters de Dirichlet}{2.09 producte sobre els caràcters de Dirichlet}{
    Sigui $m\in\ZZ/m\ZZ$, i $p$ un primer tal que $p\,\nmid\,m$, sigui $f$ l'ordre de $p$ mòdul $m$. Aleshores tenim la següent igualtat sobre $\CC[T]$:
    \[
...
\pf{
    Sigui $\psi\colon(\ZZ/m\ZZ)^{\times}\to\CC^{\times}$, aleshores pel lema \ref{Lema:2.01 caràcter de grups cíclics} tenim que $\psi(p)$ és una arrel $p$-èsima de la 
    ...
    \]
    Que envia $\theta$ a $\theta|_{\langle p\rangle}$; aleshores el que volem trobar és $\#\ker(\mathrm{Res})$. Però per la proposició \ref{Propo:2.03 restricció de caràcters de subgrups} tenim:
    \[
...
\dfe{Funció $m$-èsima de Dirichlet}{
    Sigui $m\in\NN$, aleshores la funció $m$-èsima de Dirichlet es defineix com
    \[
...
aquesta funció:
\Prop{Producte d'Euler, i sèrie de Dirichlet de $\zeta_m$.}{2.10 producte d'Euler i sèrie de zeta_m}{
    Es té:
    \begin{enumerate}
...
\pf{
    ...
    \]
    I pel lema \ref{Lema:2.09 producte sobre els caràcters de Dirichlet}, utilitzant $T=p^{-s}$:
    \[
    \prod_{p \,\nmid\, m}\left(\prod_\chi\frac{1}{1-\frac{\chi(p)}{p^s}}\right)=
    ...
    \]
    Notem que per $\s>1$, tenim que $\sum_{r\geq0}p^{-sf_pr}$ és una sèrie de Dirichlet amb coeficients $\in\{0,1\}$. A més, com que sabem que una sèrie de potències al quadrat es transforma a fer una convolució en els coeficients, sabem que ara passen a ser enters no negatius. I per tant, si considerem la funció multiplicativa que en els primers val l'enter no negatiu que hi ha al producte, (i 0 en els primers $p|m$), sabem que té tots els coeficients no 
...
\teorema{Les funcions $L(\chi,s)$ no s'anul·len en $s=1$}{2.11 les funcions L no s'anul·len a l'1}{
    \begin{enumerate}
...
\pf{
    ...
    $\zeta_m(s)$ es pot estendre a una funció holomorfa a la dreta del 0. També sabem, però, que per la proposició \ref{Propo:2.10 producte d'Euler i sèrie de zeta_m} es pot expressar com una sèrie de Dirichlet amb coeficients reals no negatius. I per tant, podem aplicar el teorema de Landau (
    \ref{Teorema:1.15 landau}), que ens diu que la sèrie de Dirichlet associada ha de convergir per $\s>0$. Però:
...
\pf{
    ...
    Però notem que $\sum_\chi\chi(a^{-1}p)=\left\{\begin{array}{cl}
        ...
        0&\text{altrament.}
    \end{array}\right.$ degut a la proposició \ref{Propo:2.06 relacions d'ortogonalitat}, utilitzant $G=(\ZZ/m\ZZ)^\times$. Per tant:
    \[
    \sum_{p\in\PP_a}\frac{1}{p^s}=\frac{1}{\varphi(m)}\sum_{\chi}\frac{f_\chi(s)}{\chi(a)}
    \]
...
\lema{Estudi de convergència de les $f_\chi$.}{2.13 Estudi de convergència de la sèrie Sum fX.}{
...
\pf{
    Notem que la diferència entre $f_\chi(s)=\sum_{p\nmid m}p^{-s}$ i $\sum_pp^{-s}$ és només en els primers termes, per tant, el comportament asimptòtic serà igual, i pel corol·lari \ref{Corolari:1.19 sèries de potències de p}, tenim:
    \[
    ...
    Però el sumatori que hi ha a la última expressió que tenim, el vam fitar en el corol·lari \ref{Corolari:1.19 sèries de potències de p}:
    \[
...
\teorema{Teorema de la progressió aritmètica de Dirichlet (fort)}{2.14 TPAD fort}{
    Sigui $a\in\ZZ$ un enter tal que $(a,m)=1$, aleshores
    \[
    \lim_{\substack{s\to 1\\s\in \RR_{>1}}}\frac{\sum_{p\in\PP_a}p^{-s}}{\log\left(\frac{1}{s-1}\right)}=\frac{1}{\varphi(m)}
    \]
}
\coro{Teorema de la progressió aritmètica de Dirichlet}{2.15 TPAD}{
    $\#P=\infty$.
}
...
\pf{
    Pel lema \ref{Lema:2.12} sabem que
    \[
    ...
    \]
    Però pel lema \ref{Lema:2.13 Estudi de convergència de la sèrie Sum fX.} tenim que en el sumatori valdrà $0$ per totes les $\chi\neq \chi_0$, i valdrà $\frac{\chi(a^{-1})}{\varphi(m)}$ si $\chi=\chi_0$, però aleshores, $\frac{\chi(a^{-1})}{\varphi(m)}=\frac{1}{\varphi(m)}$; que és el que volíem veure.
}

Notem que en aquesta demostració incloent tots els lemes previs, hem utilitzat molt el teorema de Landau, en particular el teorema \ref{Teorema:2.11 les funcions L no s'anul·len a l'1}. Però Landau no va ser el primer en demostrar això el teorema de Dirichlet. En aquest curs segurament no veurem la demostració que va donar Dirichlet al 1837-1838; però sí que veurem la que va donar Vallée-Poussin al 1896, 20 anys \footnote{No he trobat enlloc aquesta data, però em sembla que està en algun lloc dels apunts (tampoc he buscat excessivament).} abans de la de Landau; i voldrà demostrar el mateix: que $L(\chi,1)\neq 0$. 
 
\dfe{Caràcters reals i complexos.}{
    Diem que $\chi\colon\NN\to \CC$ és un caràcter de Dirichlet real si $\Ima(\chi)\subset\RR$. És a dir $\Ima(\chi)\subset\{-1,0,1\}$. I direm que és complex altrament.
}
\dfe{Caràcter conjugat}{
    Direm $\overline{X}$ al caràcter de Dirichlet conjugat
    \[
...
\Prop{$L(\chi,1)$ no s'anul·la si $\chi$ és complex.}{2.16}{
...
\Prop{$L(\chi,1)$ no s'anul·la si $\chi$ és real no principal.}{2.17}{
    Si $\chi$ és un caràcter complex, aleshores $L(\chi,1)\neq 0$.
    }
\pf{
    ...
    \]
    Però ara si definim $\sum_{n\geq1}\frac{a_n}{n^S}\vcentcolon=\prod_{\chi(p)=1}\left(1+p^{-s}\right)\left(1+p^{-s}+p^{-2s}+\dots\right)=\psi(s)$, aleshores veiem que els $a_n\geq0$ i $a_n$ és positiva amb $g_1=1$ (tot això pel lema (\ref{Propo:1.04 sèries de Dirichlet de funcions multiplicatives}) del capítol 1).\par
